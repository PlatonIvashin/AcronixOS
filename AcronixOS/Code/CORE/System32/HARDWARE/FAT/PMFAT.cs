    ...
    public static class PMFAT
    {
        ...
        public static void Initialize()
        {
            try
            {
                ...
                //foreach (var disk in Cosmos.System.FileSystem.VFS.VFSManager.GetDisks()) (ПРИГОДИТЬСЯ В БУДУЩЕМ)
                //{
        ...
        public static bool DeleteFolder(string path) // Удаление Папки
        {
            if (FolderExists(path))
            {
                try
                {
                    Console.WriteLine(en_US.adel + " " + path);
        ...
        public static Cosmos.System.FileSystem.Listing.DirectoryEntry GetFileInfo(string file)
        {
            if (FileExists(file))
            {
                ...
                {
                    Console.WriteLine(en_US.GetFileInfo);
                    Console.Write("[FSys] ", Console.ForegroundColor = ConsoleColor.Red); Console.WriteLine(ex.Message, Console.ForegroundColor = ConsoleColor.Black);
                    return null;
                }
            }
            ...
            {
                Console.WriteLine(en_US.NotLocateFile + " " + file);
                return null;
            }
        }
